...
export default function OfficialLineEditor({ resetToken, onClear }: Props) {
  ...
  const uploadImage = async (index: number, event: ChangeEvent<HTMLInputElement>) => {
    ...
    try { updateSlot(index, { uploadedImage: await blobToDataUrl(file), imageId: undefined, ...defaultImagePosition(state.slots[index].type) }); } catch { setStatus('画像を読み込めませんでした。JPEG、PNG、WebPでお試しください。'); }
    event.target.value = '';
  };
  const pickLibrary = (index: number, image: LibraryPreview) => { const type = image.postType === 'radio' || image.postType === 'health' || image.postType === 'notice' ? image.postType : state.slots[index].type; updateSlot(index, { uploadedImage: image.src, imageId: image.id, type, ...defaultImagePosition(type) }); setLibraryOpenFor(null); };
  ...
  return <>
    ...
    {state.slots.map((slot, index) => <section className="card official-line-slot" key={index}>
      ...
      <fieldset className="line-position"><legend>画像位置調整</legend><p className="hint">投稿種別に合わせた初期表示が適用されます。必要な場合のみ微調整してください。</p><label>拡大率 <output>{Math.round(slot.scale * 100)}%</output><input type="range" min="0.7" max="2" step="0.01" value={slot.scale} onChange={(e) => updateSlot(index, { scale: Number(e.target.value) })} /></label><label>左右 <output>{slot.offsetX > 0 ? '+' : ''}{slot.offsetX}</output><input type="range" min="-100" max="100" value={slot.offsetX} onChange={(e) => updateSlot(index, { offsetX: Number(e.target.value) })} /></label><label>上下 <output>{slot.offsetY > 0 ? '+' : ''}{slot.offsetY}</output><input type="range" min="-100" max="100" value={slot.offsetY} onChange={(e) => updateSlot(index, { offsetY: Number(e.target.value) })} /></label><button onClick={() => updateSlot(index, defaultImagePosition(slot.type))}>初期位置に戻す</button></fieldset>
