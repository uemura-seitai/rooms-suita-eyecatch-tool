...
export type TextStyle = { size: number; family: string; color: string; weight: 400 | 500 | 700 };
...
export async function render(canvas: HTMLCanvasElement, template: Template, values: { text1: string; text2: string; subtitle: string; tag1: string; tag2: string; label: string; text1Style: TextStyle; text2Style: TextStyle; verticalOffset: number; uploaded?: HTMLImageElement }) {
  ...
  // Remove temporary health placeholders, sampling the adjacent template color so its artwork remains intact.
  if (template.type === 'health') { ctx.save(); const band = ctx.getImageData(100, 525, 1, 1).data; ctx.fillStyle = `rgb(${band[0]},${band[1]},${band[2]})`; [[355, 516, 135, 58], [540, 516, 145, 58]].forEach(([x, y, w, h]) => ctx.fillRect(x, y, w, h)); (template.placeholderBoxes ?? []).forEach((b) => { const pixel = ctx.getImageData(b.x + 8, b.y + 8, 1, 1).data; ctx.fillStyle = `rgb(${pixel[0]},${pixel[1]},${pixel[2]})`; ctx.fillRect(b.x, b.y, b.width, b.height); }); ctx.restore(); }
  ...
  if (template.type === 'notice') {
    ...
    const layout = layoutText(ctx, values.text1, template.titleBox.width, values.text1Style.size, 24, template.title.maxLines, values.text1Style); const y = template.titleBox.y + (template.titleBox.height - layout.height) / 2 + values.verticalOffset;
    drawLines(ctx, layout, template.titleBox.x, y, values.text1Style, 'left');
  }
  if (template.type === 'health') { ctx.font = `500 32px ${fontFamily}`; ctx.textAlign = 'left'; if (values.subtitle && template.subtitleBox) ctx.fillText(values.subtitle, template.subtitleBox.x, template.subtitleBox.y, template.subtitleBox.width); const tags = [values.tag1, values.tag2].filter(Boolean).map((tag) => `#${tag.replace(/^#/, '')}`).join('　'); ctx.font = `700 24px ${fontFamily}`; if (tags && template.tagsBox) ctx.fillText(tags, template.tagsBox.x, template.tagsBox.y, template.tagsBox.width); if (template.labelBox && values.label) { ctx.fillStyle = '#333333'; ctx.font = `700 29px ${fontFamily}`; ctx.fillText(values.label, template.labelBox.x, template.labelBox.y, template.labelBox.width); } }
}
